...
El* parse_file(char* file_path, callback* callback) {
    ...
    *c = fgetc(stream);
    ...
    if (*c == '?') {
        ...
        exit = report_comment(c, stream, "", callback); // report initial comment
        ...
        while (*c == '<') {
            *c = fgetc(stream);
            if (*c == '/') // found a closing tag
                ...
                exit = report_comment(c, stream, el->path, callback);
                if (exit == EXIT_FAILURE) 
                    return EXIT_FAILURE;
...
int report_any_comments(char* c, FILE* stream, char* path, callback* callback) {
    ...
    while (*c == '<') {
        *c = fgetc(stream);
        ...
        int exit = report_comment(c, stream, path, callback);
        if (exit == EXIT_FAILURE) 
            return EXIT_FAILURE;
...
int report_comment(char* c, FILE* stream, char* path, callback* callback) {
